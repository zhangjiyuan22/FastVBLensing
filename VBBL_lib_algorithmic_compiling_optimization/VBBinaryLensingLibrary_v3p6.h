...
public: 
	...
	_theta * insert_at_certain_position(_theta *, double) ;
							...
							//         (O(1) complexity)
	/*******************************************   end   *******************************************/
	void remove(_theta*);   // method: remove and delete the element pointed by the input pointer, 
...
class _point{									  // a total of 128 bytes per object, 2 cache lines
public:
	double x1;                                    // this point's coordinate
	double x2;
	double parab,ds,dJ, parabastrox1 ; 			  // ds is x'^x" at this point
...
public:
	...
	/******************************************* changed *******************************************/
	_curve *divide(_point *, int);	// method: used only one time in OrderImages, just next to the hot spot 'while loop';
									//
...
/*
void push_heap(double * __restrict heap_array, int heap_size, double value_to_push)
{	
	//heap_array.push_back(value_to_push) ; 
	//int hole_index = heap_array.size() - 1 ; 
	int hole_index = heap_size ; 						// new_heap_size - 1, aka original heap_size
	int parent_index = (hole_index - 1) / 2 ; 

	while (hole_index > 0 && heap_array[parent_index] < value_to_push)
	{
	heap_array[hole_index] = heap_array[parent_index] ;
	hole_index = parent_index ;
	parent_index = (hole_index - 1) / 2 ;
	}
	
	heap_array[hole_index] = value_to_push ;
}

void pop_heap(double * __restrict heap_array, int heap_size)
{
	int last_index = heap_size - 1 ; 
	double value = heap_array[last_index] ; 
	last_index-- ;
	//heap_array.pop_back() ;
	int hole_index = 0 ;
	...
	while(true)
	{
		...
		if (right_child_index <= last_index && heap_array[right_child_index] > heap_array[max_child_index])
			max_child_index = right_child_index ; 		// now, max child becomes right child

		if (value >= heap_array[max_child_index])
			break ; 
		heap_array[hole_index] = heap_array[max_child_index] ; 
		hole_index = max_child_index ; 
	}

	heap_array[hole_index] = value ;
}


void pop_then_push_heap(double * __restrict heap_array, int heap_size, double value_to_push)
{
	int last_index = heap_size - 1 ; 
	int hole_index = 0 ;
	...
	while(true)
	{
		...
		if (value_to_push >= heap_array[max_child_index])
			break ; 
		heap_array[hole_index] = heap_array[max_child_index] ; 
		hole_index = max_child_index ; 
	}
