...
#endif
/* if we want to inline functions, we can not just add inline in front of each function defination like in VBBL, 
   because VBBL has only 1 source file,
   while VBMicrolensing has multiple source files, each file is first compiled and linked together, 
   so if we still define inline functions in one of the source file, it can not be inlined.
   so we need to put the defination of inline functions into header file, 
   then each source file can see the defination of inline functions during the compilation period 
*/
