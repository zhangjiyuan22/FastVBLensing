...
void VBMicrolensing::SetLensGeometry_multipoly(int nn, double* q, complex* s) {
	...
	// lens with lowest mass is now placed in q_sort[0] and s_sort[0]
	for (j = 0; j < x - 1; j++) {
		do {
			k = 0;
			for (i = 0; i < x - 1; i++) {
	...
	//normalize q_sort array s.t. sum(q_sort[i])=1.0
	for (i = 0; i < n; i++) {
		q_sort[i] = q_sort[i] / sumq;
	}

	//set all reference systems, with the nth smaller mass in the first place
	// m_mp/a_mp[0][i] has lowest mass lens as 0th element, and a_mp[0][i] are positions relative to lowest mass lens 
	//						123												   (taking the lowest mass lens as origin)
	// m_mp/a_mp[1][i] has 2nd-lowest mass lens as 0th element, lowest mass lens as 1st element, 3rd-lowest mass lens as 2nd element
	//                      213                                    a_mp[1][i] are positions relative to 2nd-lowest mass lens 
	//																		   (taking the 2nd-lowest mass lens as origin)
	// m_mp/a_mp[2][i] has 3rd-lowest mass lens as 0th element, 2nd-lowest mass lens as 1st element, lowest mass lens as 2nd element
	//                      321                                    a_mp[1][i] are positions relative to 3rd-lowest mass lens 
	//																		   (taking the 3rd-lowest mass lens as origin)
	// intrinsically, swap the lowest mass lens and the nth-lowest mass lens, while retaining other lenses' sequence
	...
	*s_offset = s_sort[0];
	...
	// pmza_mp: array of n polynomials m[i] * Product(z-a[j]) (polynomials of degree n-1)
	// pza_mp : is Product(z-a[i]) (a polynomial of degree n)
	...
	for (int k = 0; k < n; k++) {
		...
		for (int i = 0; i < n; i++) 
		{
			...
			for (int j = 0; j < n; j++) 
			{
				if (i == j) 
				{
					copypol(pza_mp[k], i, pdum);              // pdum[0...i]        = pza_mp[k][0...i]
					polyproduct(pdum, i, pbin, 1, pza_mp[k]); // pza_mp[k][0...i+1] = pdum[0...i] * pbin[0...1]
					//essentially change pza_mp[k][0...i+1] to pza_mp[k][0...i] * (z - a_mp[k][i])
				}
				else 
				{
					dg = (j > i) ? i : i - 1;
					copypol(pmza_mp[k][j], dg, pdum);              // pdum[0...dg]            = pmza_mp[k][j][0...dg]
					polyproduct(pdum, dg, pbin, 1, pmza_mp[k][j]); // pmza_mp[k][j][0...dg+1] = pdum[0...dg] * pbin[0...1]
					//essentially change pmza_mp[k][j][0...dg+1] to pmza_mp[k][j][0...dg] * (z - a_mp[k][i])
				}
			}
			//123, for i=0, first j=0, change pza_mp [k][0...1]    to pza_mp [k][0](=1.0) * (z - a_mp[k][i]),
			//              then  j=1, change pmza_mp[k][j][0...1] to pmza_mp[k][j][0](=m_mp[k][j]) * (z - a_mp[k][i])
			//              then  j=2, change pmza_mp[k][j][0...1] to pmza_mp[k][j][0](=m_mp[k][j]) * (z - a_mp[k][i])
			//              finally pmza_mp[k][1][0...1] = m_mp[k][1] * (z - a_mp[k][0])
			//						pmza_mp[k][2][0...1] = m_mp[k][2] * (z - a_mp[k][0])
			//						pza_mp [k][0...1]    =              (z - a_mp[k][0])
			//     for i=1, first j=0, change pmza_mp[k][j=0][0...1] to pmza_mp[k][j=0][0](=m_mp[k][j=0]) * (z - a_mp[k][i])
			//              then  j=1, change pza_mp [k][0...2]      to pza_mp [k][0...1](=z - a_mp[k][0]) * (z - a_mp[k][i])
			//              then  j=2, change pmza_mp[k][j=2][0...2] to pmza_mp[k][j=2][0...1] * (z - a_mp[k][i])
			//              finally pmza_mp[k][0][0...1] = m_mp[k][0] * (z - a_mp[k][1])
			//						pmza_mp[k][2][0...2] = m_mp[k][2] * (z - a_mp[k][0]) * (z - a_mp[k][1])
			//						pza_mp [k][0...2]    =              (z - a_mp[k][0]) * (z - a_mp[k][1])
			//     for i=2, first j=0, change pmza_mp[k][j=0][0...2] to pmza_mp[k][j=0][0...1] * (z - a_mp[k][i])
			//              then  j=1, change pmza_mp[k][j=1][0...2] to pmza_mp[k][j=1][0...1] * (z - a_mp[k][i])
			//              then  j=2, change pza_mp [k][0...3]      to pza_mp [k][0...2] * (z - a_mp[k][i])
	...
	// lack for __restrict
	// leading to unnecessary memory access?
}
