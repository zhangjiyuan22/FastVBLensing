...
#define _Jacobians4\
	...
	cq=(dJ.re<-100)? 0.0 : sqrt(J3.re*J3.re+J3.im*J3.im);
	/******************************************* changed *******************************************/
...
/******************************************* changed *******************************************/
double VBMicrolensing::MultiMag(complex yi, double RSv, double Tol, _sols_for_skiplist_curve **Images) {
...
/*******************************************   end   *******************************************/
	...
	try {
		...
		/******************************************* changed *******************************************/
		// this block seems not completed: jtheta not used

		// Let us check the number of images in a square around the source
		// if (squarecheck) {
		// 	itheta = Thetas->first;
		// 	stheta = new _theta(-1);
		// 	while (itheta->next) {
...
//				}
		...
		while (((currerr > errimage) && (currerr > RelTol * Mag) && (NPS < NPSmax) && (flag < NPSold))) {
			...
			/******************************************* changed *******************************************/
			// the property: 'only insert when delta th > 1e-8' is re-achieved by the above code
			// thus we can safely comment below code
...
/*******************************************   end   *******************************************/
	...
	for (int i = 0; i < Sols->length; i++) {
		if (th < scurve->first->theta->th) {
			if (th > scurve->first->theta->prev->prev->th) { // sembra strano ma � giusto prev->prev->th (l'inserimento di theta c'� gi� stato)
				cfoll[nfoll] = scurve; // immagine coinvolta all'inizio
				nfoll++;
				scurve2 = scurve->next;
				Sols->drop(scurve);
	...
	// Constructing matrix distances between images in creation
	mi = 1.e100;
	scan = Newpts->first;
	for (int i = 0; i < npres - 1; i++) {
		scan2 = scan->next;
	...
	// construct distance matrix between destructed images 
	mi = 1.e100;
	for (int i = 0; i < nprec - 1; i++) {
		for (int j = i + 1; j < nprec; j++) {
