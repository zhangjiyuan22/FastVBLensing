...
double VBBinaryLensing::BinaryMag0(double a1, double q1, double y1v, double y2v, _sols **Images) {
	...
	// checkpoint (jump between BinaryMag0 and NewImages by finding this string in VScode)
...
////////////////////////////  
	...
	do {
		...
		if (Prov->length > 0) {
			flagbad = 0;
			OrderImages((*Images), Prov); 				// _sols variable pointed by *Images can have 3 _curve variables
														// while _curve variable pointed by 'Prov' have 5 _point variables, 
														// or vice versa?
		...
		if (flagbad == 0 || flagbad == flagbadmax) {
			...
			while (stheta->next) {              /****************************************************************************************/
												...
												// because complex_lagguerre2newton is more time-consuming in triple?)
												//
...
#define _Jacobians4\
	...
	cq=std::sqrt(J3.re*J3.re+J3.im*J3.im);
...
void VBBinaryLensing::OrderImages(_sols *Sols, _curve *Newpts) {
	...
	th = theta->th;
	theta->Mag = theta->prev->Mag = theta->maxerr = theta->prev->maxerr = 0; // two segments need to be updated after inserting one theta
	...
	if (Newpts->length == 3) {
		...
		if ((mi>theta->errworst) && (theta->next->errworst>0.)) {// if step i+1 source is not inside the caustic, add an error
			theta->maxerr = mi*mi;
		}
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////astro: destructed image:
	...
	while (nprec) {
		...
		parab1 = (scan->ds + scan2->ds)*mi; // Old parabolic correction
...
_curve *_curve::join(_curve *nc) {	     // method: used once in OrderImages (actually also used one time in PlotCrit
	...
	partneratend = nc->partneratend;      				 // set current curve's attribute 'parner_at_end' to nc's
	if (partneratend) partneratend->partneratend = this; // redirect nc->parner_at_end's attribute 'parner_at_end' from 'nc' to current curve
...
void _thetas::remove(_theta* stheta) { // has no return value
	...
	while (scan!=0) {              // only last's 'next'==0
		...
		scan = scan->next;         // when scan==last, then scan = last->next = 0, then while loop terminates
								   ...
								   // why not update 'first'/'last' when stheta == 'first'/'last'? (although we know we won't delete 0 and 2*PI)
}
